        ...
        public static bool Prefix(NetworkedKillBehaviour __instance, PlayerRef targetedPlayer, PlayerRef perpetrator, ref int action)
        {
            ...
            foreach (var role in SubRole.All)
            {
                switch (role)
                {
                    case Arsonist:
                    {
                        if (role.PlayerWithRole != null && action == (int)ProximityTargetedAction.Kill && role.PlayerWithRole.KnownGameRole != GameRole.Sheriff)
                        {
                            try
                            {
                        ...
                        if (action == (int)ProximityTargetedAction.Vote) return true;
                        break;
                    }
                    case Sniper:
                    {
                    ...
                    case Witch witch:
                    {
                        if (witch.PlayerWithRole != null && action == (int)ProximityTargetedAction.Kill  && witch.PlayerWithRole.KnownGameRole != GameRole.Sheriff)
                        {
                            try
                            {
                    ...
                    case Duelist duelist:
                    {
                        ...
                        if (other != null && action == (int)ProximityTargetedAction.Kill && role.PlayerWithRole.KnownGameRole != GameRole.Sheriff || other.PlayerWithRole.KnownGameRole != GameRole.Sheriff)
                        {
                            try
                            {
                    ...
                    case Silencer silencer when silencer.PlayerWithRole != null && silencer.CanMutePlayer && action == (int)ProximityTargetedAction.Kill && silencer.PlayerWithRole.SoulLinkID == -1 && role.PlayerWithRole.KnownGameRole != GameRole.Sheriff:
                        ...
                        return false;
                    case Silencer silencer when silencer.PlayerWithRole != null && !silencer.CanMutePlayer && action == (int)ProximityTargetedAction.Kill && silencer.PlayerWithRole.SoulLinkID == silencer.PlayerWithRole.PlayerId && role.PlayerWithRole.KnownGameRole != GameRole.Sheriff:
